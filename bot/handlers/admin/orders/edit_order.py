...
from ....keyboards.reply import order_status_kb
...
class EditOrderFlow(StatesGroup):
    ORDER_ID = State()
    NEW_VALUE = State()
...
async def start_edit_order(
    ...
    text = _("order_edit_id", language_db)
    ...
    if state:
        await EditOrderFlow.ORDER_ID.set()
        await state.set_state(EditOrderFlow.ORDER_ID.state)
...
async def process_order_id(msg: Message, state: FSMContext):
    ...
    if order_id == "/start":
        ...
        if order_data is None or order_id == "0":
            text = (
                f"{_('order_edit_unknown', language_db)}" f"{_('again', language_db)}"
            )
            await msg.answer(text)
            return
        ...
        text = (
            ...
            + f"„{comment}“\n\n"
            + f"{_('status_new', language_db)}"
        )

        await msg.delete()
        await msg.answer(text, reply_markup=order_status_kb())
        await EditOrderFlow.NEW_VALUE.set()
...
async def process_new_value(msg: Message, state: FSMContext):
    ...
    order = Order()
    if new_value not in ["⌛️", "📦", "✅"]:
        text = f"{_('edit_params', language_db)}" f"{_('again', language_db)}"
        await msg.answer(text)
        return

    order.update(order_id, {"status": new_value})

    await msg.answer(
        _("order_edit_success", language_db), reply_markup=back_order_kb(user_id)
    )
    await state.finish()
...
def register_edit_order_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(
        start_edit_order, lambda c: c.data == "edit_order", state="*"
    )
    dp.register_message_handler(start_edit_order, commands=["edit_order"], state="*")
    dp.register_message_handler(process_order_id, state=EditOrderFlow.ORDER_ID)
    dp.register_message_handler(process_new_value, state=EditOrderFlow.NEW_VALUE)
